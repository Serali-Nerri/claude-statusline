...
export function renderSessionLine(ctx) {
    ...
    if (display?.showUsage !== false && ctx.usageData && !shouldHideUsage(ctx.stdin)) {
        ...
        if (isLimitReached(ctx.usageData)) {
            ...
            if (usageCompact) {
                usageParts.push(critical(`⚠ Limit${resetTime ? ` (${resetTime})` : ''}`, colors));
            }
            else {
                const resetSuffix = resetTime
                    ? showResetLabel
                        ...
                        : ` (${resetTime})`
                    : '';
                usageParts.push(critical(`⚠ ${t('status.limitReached')}${resetSuffix}`, colors));
            }
        }
        else {
    ...
    if (display?.showSessionTokens && ctx.transcript.sessionTokens) {
        ...
        if (total > 0) {
            parts.push(`\x1b[38;5;245m${t('format.tok')}: ${formatTokens(total)} (${t('format.in')}:${formatTokens(st.inputTokens)}, ${t('format.out')}: ${formatTokens(st.outputTokens)})${RESET}`);
        }
    }
    ...
    const systemTime = new Date().toLocaleTimeString('en-GB', { hour12: false });
    parts.push(`\x1b[38;5;245m${systemTime}${RESET}`);
    let line = parts.length > 1 ? `${parts[0]}| ${parts.slice(1).join(' | ')}` : parts.join(' | ');
